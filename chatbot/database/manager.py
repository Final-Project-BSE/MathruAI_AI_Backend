...
class DatabaseManager:
    ...
    def get_chat_session_title(self, session_id: int, user_id: str) -> Optional[str]:
        """Get the title of a user's chat session."""
        if not self.connection:
            return None
        ...
        try:
            ...
            logger.error(f"Error getting chat session title: {e}")
            return None

    def update_chat_session_title(self, session_id: int, user_id: str, session_name: str) -> bool:
    ...
    def get_chat_session_message_count(self, session_id: int, user_id: str) -> int:
        """Get current message count for a user's chat session."""
        if not self.connection:
            return 0
        ...
        try:
            ...
            logger.error(f"Error getting chat session message count: {e}")
            return 0
